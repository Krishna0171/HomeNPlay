      ...
      if (response.functionCalls && response.functionCalls.length > 0) {
        for (const fc of response.functionCalls) {
          if (fc.name === 'navigateToPage') {
            ...
            // Map typical AI outputs to routes
            const routeMap: Record<string, string> = {
              ...
              'contact': '/contact',
              'favorites': '/favorites',
              'admin-dashboard': '/' // Placeholder or actual route if exists
            };
  ...
  return (
    <div className="fixed bottom-6 left-6 sm:bottom-8 sm:left-auto sm:right-8 z-[100] flex flex-col items-end">
        ...
        <div className="mb-4 w-[calc(100vw-3rem)] sm:w-96 h-[550px] bg-white rounded-[2.5rem] shadow-2xl border border-slate-100 flex flex-col overflow-hidden animate-in slide-in-from-bottom-10 fade-in duration-300">
          ...
          <div className="bg-indigo-600 p-6 text-white flex items-center justify-between shadow-lg">
            <div className="flex items-center space-x-3">
              ...
              <div>
                <h3 className="font-bold">HomeNPlay Assistant</h3>
          ...
          >
            {messages.map((m, i) => (
              ...
              >
                <div className={`max-w-[90%] p-4 rounded-2xl text-sm ${m.role === 'user'
          ...
          <form onSubmit={handleSend} className="p-4 bg-white border-t border-slate-100">
            ...
            <p className="text-[9px] text-center text-slate-400 mt-2 uppercase tracking-tighter font-medium">HomeNPlay AI Helper</p>
          </form>
        </div>
      )}
      ...
      <button
        ...
        className={`h-16 w-16 rounded-full flex items-center justify-center shadow-2xl transition-all duration-500 hover:scale-110 active:scale-95 group ${isOpen ? 'bg-slate-900 text-white rotate-90' : 'bg-indigo-600 text-white'
          }`}
      >
        {isOpen ? (
          <X className="h-7 w-7" />
