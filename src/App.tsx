...
const App: React.FC = () => {
  ...
  const [loadingMessage, setLoadingMessage] = useState('Welcome to HomeNPlay...');
  ...
  const executeOrder = async (shippingAddress: Address, paymentMethod: PaymentMethod) => {
    ...
    try {
      ...
      const message = `*NEW ORDER - HomeNPlay*\n\n` +
    ...
    <div className="min-h-screen bg-slate-50 flex flex-col">
      ...
      {notification && (
        <div className="fixed bottom-6 right-6 z-[100] animate-in slide-in-from-right-10">
          <div className={`flex items-center space-x-3 px-6 py-4 rounded-2xl shadow-2xl border ${notification.type === 'success' ? 'bg-emerald-600 border-emerald-500 text-white' : 'bg-red-600 border-red-500 text-white'
            }`}>
      ...
      <main className="flex-1">
        <Routes>
          <Route path="/" element={<Home onAddToCart={addToCart} favorites={favorites} onToggleFavorite={toggleFavorite} onOpenChat={() => setIsChatOpen(true)} />} />
          <Route path="/product/:id" element={<ProductDetails onAddToCart={addToCart} favorites={favorites} onToggleFavorite={toggleFavorite} />} />
          <Route path="/cart" element={<Cart items={cart} user={user} initialAddress={checkoutAddress} onUpdateQuantity={updateCartQuantity} onRemove={removeFromCart} onCheckout={handleCheckout} />} />
          <Route path="/favorites" element={<Favorites onAddToCart={addToCart} favorites={favorites} onToggleFavorite={toggleFavorite} />} />
          <Route path="/profile" element={user ? <Profile user={user} onUpdate={setUser} showNotification={showNotification} /> : <div />} />
          <Route path="/my-orders" element={user ? <MyOrders userId={user.id} /> : <div />} />
          <Route path="/order/:id" element={<OrderDetails />} />
          <Route path="/faq" element={<FAQ />} />
          <Route path="/about" element={<AboutUs />} />
          <Route path="/contact" element={<ContactSupport onOpenChat={() => setIsChatOpen(true)} />} />
      ...
      <footer className="bg-white border-t border-slate-200 py-12 mt-20">
        <div className="max-w-7xl mx-auto px-6 text-center text-slate-400 text-xs">
          &copy; {new Date().getFullYear()} HomeNPlay. All rights reserved.
        </div>
      </footer>
    </div>
  );
